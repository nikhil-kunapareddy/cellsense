...
- Default model is `groq:openai/gpt-oss-120b`. Groq, Gemini, and Anthropic are verified
  working here; OpenAI is implemented but **unverified** (no key on this machine).
- **Anthropic rejects `temperature`** on the Claude 5 generation (Fable 5.x, Opus 5,
  Sonnet 5) and on Opus 4.7/4.8 — a 400 on every call. `langchain_anthropic` does *not*
  drop it for you: `anthropic>=1` stopped accepting sampling params as named arguments, so
  `_sdk_compat` relocates them into `extra_body`, which the SDK merges into the request
  JSON verbatim. The parameter is therefore absent from the payload dict and still on the
  wire. `providers/anthropic.py` omits it per model via `_ACCEPTS_SAMPLING_PARAMS`, and
  unknown ids omit it too — the failure modes aren't symmetric. Don't "simplify" that back
  into an unconditional pass-through to match the other four providers.
